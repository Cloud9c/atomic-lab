        ...
        window.requestAnimationFrame(function() {
            ...
            for (let i = 0; i < selectedMolecules.length; i++) {
                ...
                for (let j = 0; j < sm.length; j++) {
                    ...
                    line.setAttribute("x1", (+line.getAttribute("x1")) + x);
                    line.setAttribute("x2", (+line.getAttribute("x2")) + x);
                    line.setAttribute("y1", (+line.getAttribute("y1")) + y);
                    line.setAttribute("y2", (+line.getAttribute("y2")) + y);
                }
                ...
                if (document.getElementById("main").getElementsByClassName("selected").length === 2) {
                    ...
                    if (!dataLine1 || !dataLine2 || (dataLine1 && dataLine2 && !dataLine1.split(";").some(r => dataLine2.split(";").includes(r)))) {
                        ...
                        document.getElementById("menu-options").getElementsByClassName("option")[1].onclick = function() {
                            const lineCounter = (+localStorage.getItem("lineCounter")) + 1;
...
function createElement(target, x, y, table) {
    ...
    function hotbarMouseUp() {
        ...
        const elementCounter = (+localStorage.getItem("elementCounter")) + 1;
...
document.getElementById("react").addEventListener("click", function() {
    ...
    if (animate) {
        ...
        for (let i = 0; i < document.getElementById("main").getElementsByClassName("element").length; i++) {
            document.getElementById("main").getElementsByClassName("element")[i].style.transform = "translate(" + 0 + "px, " + 0 + "px)";
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            currentPos.push([+currentPosition[0], +currentPosition[1]]);
        }
    } else {
    ...
    function frame() {
        for (let i = 0; i < document.getElementById("main").getElementsByClassName("element").length; i++) {
            elements[i].style.transform = "translate(" + ++currentPos[i][0] + "px," + ++currentPos[i][1] + "px)";
        }
        animate = window.requestAnimationFrame(frame);
    }
});

document.getElementById("periodic-table").addEventListener("mouseover", function() {
...
window.addEventListener("beforeinstallprompt", function(e) {
    ...
    document.getElementById("install").addEventListener("click", function() {
        installApp.prompt();
        installApp.userChoice.then(function() {
